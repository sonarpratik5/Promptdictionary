...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  return <form action={formAction} className="mt-8 grid gap-5">
    ...
    <p className="text-center text-sm text-foreground-muted">{isSignIn ? "New here?" : "Already have an account?"} <Link href={isSignIn ? "/auth?mode=sign-up" : "/auth"} className="font-bold text-accent hover:underline">{isSignIn ? "Create an account" : "Sign in"}</Link></p>
