...
export default async function AuthPage({ searchParams }: { searchParams: Promise<{ mode?: string; error?: string; reset?: string; next?: string }> }) {
  ...
  return <main id="main-content" tabIndex={-1} className="mx-auto grid max-w-6xl gap-10 px-5 py-12 md:grid-cols-[.9fr_1.1fr] md:py-20">
    ...
    <section className="card p-6 md:p-10" aria-labelledby="auth-heading">
      ...
      {!isSupabaseConfigured() ? <div className="alert-notice mt-6"><strong>Sign-in is waiting for the project connection.</strong><br />Add the Supabase project URL and anonymous key to the local environment, then restart the development server. You can still explore and adapt every prompt in the library.</div> : <AuthForm key={`${mode}-${params.next ?? ""}`} mode={mode} next={params.next} />}
    </section>
  </main>;
}
